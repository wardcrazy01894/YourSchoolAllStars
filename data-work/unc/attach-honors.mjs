...
//
// Reads honors/ledger-guide.json + honors/ledger-wiki.json, matches each
// entry to a player-season in src/data/unc-football.json (normalized name +
...
const DATASET = join(HERE, '..', '..', 'src', 'data', 'unc-football.json')
...
const NAME_MAP = {
  // honors-page spelling → dataset spelling (verified same human/season)
  'dre bly': 'dre bly',
  'anthony ratliff-williams': 'a ratliff',
  'a ratliff': 'a ratliff',
}
const norm = (s) =>
  s
...
const ledger = [
  ...JSON.parse(readFileSync(join(HERE, 'honors', 'ledger-guide.json'))),
